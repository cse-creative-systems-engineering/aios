...
fn app_binary() -> PathBuf {
    if let Ok(p) = std::env::var("AIOS_APP_BIN") {
...
async fn file_artifact_via_ui() {
    ...
    let driver = Command::new("tauri-driver")
        ...
        .spawn()
        .expect("tauri-driver");
    let webkit = Command::new("WebKitWebDriver")
        .args(["--port=4445"])
        .spawn()
        .expect("webkit");
    ...
    let app = Command::new(app_binary())
    ...
    match prompt {
        Ok(el) => {
            el.send_keys("create a file hello.py with hello world")
                .await
                .unwrap();
            let form = client
                .find(Locator::Css("#prompt-form button[type='submit']"))
                .await
                .unwrap();
            ...
            println!(
                "body len {} contains artifact={}",
                body.len(),
                body.contains("Artifact")
            );
            assert!(
                body.contains("Artifact"),
                "artifact card should be in source"
            );
        }
        Err(e) => {
