...
fn harness_repro_file_write() {
    ...
    let r = coord.run_tool("files.write_file", "file:/workspace/hello.py hello from aios");
    match &r {
    ...
    assert!(text.contains("committed=true"), "expected committed, got {}", text);
    assert!(target.exists(), "file not on disk at {:?}", target);
    let content = std::fs::read_to_string(&target).unwrap();
    assert!(content.contains("hello from aios"), "content mismatch {}", content);
    println!("HARNESS OK: staged->committed file:/workspace/hello.py -> {:?}", target);
}
...
fn harness_direct_broker() {
    ...
    let cap = Capability { resource: ResourceId("file:/workspace".into()), operation: Operation::Write };
    let token = aios::capability::CapabilityToken {
        ...
        granted_at: 1000,
        expires_at: 999999,
        provenance: aios::capability::Provenance { granted_by: PrincipalId::system("policy-broker"), package_id: "files.specialist".into(), package_version: 1, signature_verified: true },
    };
    broker.register_tool(aios::capability::ToolDefinition { tool_id: "files.write_file".into(), specialist_package: "files.specialist".into(), risk_level: RiskLevel::Staged, required_capabilities: vec![cap.clone()], description: "write".into() });
    broker.register_principal(principal.clone(), vec![cap.clone()], Clearance(RiskLevel::Recovery));
    broker.set_resource_state(ResourceId("file:/workspace".into()), aios::capability::ResourceState::Available);
    ...
    broker.set_executor(aios::executor::StagedExecutor::new(Box::new(store), Box::new(FileDriver::with_roots(ws.clone(), arts))));
    let req = aios::broker::build_request(principal.clone(), ResourceId("file:/workspace/hello.py".into()), Operation::Write, "files.write_file", &token, aios::protocol::ToolParameters::Stage { change: serde_json::json!({"content": "harness content"}) }, uuid::Uuid::new_v4(), 10);
