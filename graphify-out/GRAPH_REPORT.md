...
## Corpus Check
- 152 files · ~276,730 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2829 nodes · 7282 edges · 233 communities (105 shown, 128 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 89 edges (avg confidence: 0.78)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `acc0f3af`
...
- `node()` --calls--> `t()`  [INFERRED]
  src/tools.rs → src/graph.rs
- `kernel_module_request()` --calls--> `ResourceId`  [INFERRED]
  src/main.rs → src/capability.rs
- `register_principals()` --calls--> `ResourceId`  [INFERRED]
  src/main.rs → src/capability.rs
...
## Communities (233 total, 128 thin omitted)
...
Cohesion: 0.07
Nodes (55): Default, EdgeMetadata, EdgeType, FnOnce, NodeId, NodeMetadata, NodeType, Sized (+47 more)

### Community 2 - "NodeId"
Cohesion: 0.10
Nodes (41): node(), Timestamp, add_edge_requires_both_endpoints(), add_node_rejects_duplicate(), dependencies_and_dependents_track_both_directions(), edge(), EdgeMetadata, EdgeProvenance (+33 more)

### Community 3 - "protocol.rs"
Cohesion: 0.07
Nodes (61): ApprovalId, AuditEntryId, CheckpointRef, MessageId, PlanId, RequestId, error_code_for(), ActionPlan (+53 more)

### Community 4 - "tests.rs"
Cohesion: 0.07
Nodes (72): CapabilityToken, Clearance, Operation, PrincipalId, ResourceId, RiskLevel, tool_arguments(), Coordinator (+64 more)
...
Cohesion: 0.12
Nodes (26): diagnose_flags_missing_signature_evidence(), discovers_package_nodes(), exposes_only_read_only_tools(), health_counts_signature_evidence(), instantiates_with_owns_edges_for_each_resource(), is_package_node(), not_found(), observe_reports_domain_metrics() (+18 more)

### Community 26 - "PrincipalId"
Cohesion: 0.07
Nodes (28): Fn, PolicyBroker, Box, Capability, Default, HashSet, Option, ProgressSink (+20 more)

### Community 27 - "boot.rs"
Cohesion: 0.13
Nodes (25): boot_graph(), BootRecoveryError, BootRecoveryHealth, BootRecoverySpecialist, diagnose_flags_unhealthy_nodes(), discovers_boot_nodes(), exposes_only_read_only_tools(), health_counts_healthy_nodes() (+17 more)
...
Cohesion: 0.10
Nodes (26): Arc, AuditLog, BootRecoverySpecialist, Broker, DriversSpecialist, GraphicsSpecialist, GraphPhase, MemorySpecialist (+18 more)
...
### Community 37 - "String"
Cohesion: 0.24
Nodes (10): AgentError, DiscoveredModel, ModelEntry, ChatOutcome, ProviderCatalogue, providers_text(), String, ToolResult (+2 more)
...
### Community 39 - ".submit"
Cohesion: 0.13
Nodes (10): AtomicBool, AtomicU32, DataClassification, ConsentRecord, MockBackend, ProviderTier, Mutex, Option (+2 more)

### Community 40 - "String"
Cohesion: 0.23
Nodes (8): Into, Self, FinishReason, GatewayResponse, GenerationError, GenerationResponse, LinuxConnectivityProbe, String
...
### Community 51 - "ModelBackend"
Cohesion: 0.67
Nodes (4): Send, ConnectivityProbe, ModelBackend, Sync

### Community 52 - "ui_e2e.rs"
Cohesion: 0.22
Nodes (18): Child, Client, Drop, app_binary(), assert_surface(), ChildGuard, dump_windows(), Option (+10 more)
...
Cohesion: 0.20
Nodes (17): format_review(), garbage_becomes_freeform(), loose_review(), parse_review(), parses_approve(), parses_approve_with_conditions(), parses_reject(), review_formats_verdict() (+9 more)
...
Cohesion: 0.20
Nodes (15): Coordinator, operation_for_tool(), protocol_tool_result(), quote_value(), required_specialist_calls(), Option, Result, String (+7 more)
...
### Community 64 - "ProviderId"
Cohesion: 0.13
Nodes (10): Display, Graphics Specialist, Error, Formatter, GPU, Session, GatewayError, ProviderId (+2 more)

### Community 65 - "AgentError"
Cohesion: 0.18
Nodes (13): AgentError, Planner, Arc, Display, Error, Formatter, From, Result (+5 more)
...
Cohesion: 0.13
Nodes (15): AiosConfig, Box, ConfigError, ConnectivityProbe, RegistryError, RoutingError, BootError, config_dir_for() (+7 more)
...
### Community 77 - "ModelRegistry"
Cohesion: 0.15
Nodes (10): HashMap, Item, Iterator, RwLock, FakeProbe, missing_local_model_marks_provider_unhealthy(), ConnectivityState, ModelRegistry (+2 more)
...
### Community 79 - "model.rs"
Cohesion: 0.11
Nodes (26): Duration, HealthState, AgentRole, combine(), DataPolicy, deregister_provider_drops_only_that_provider(), has_default_route(), has_default_route_v6() (+18 more)

### Community 81 - "Aios"
Cohesion: 0.07
Nodes (36): Archive README, Current State, Grounding Snapshot: Coordinator Modularization, Surface Harness, and Graphify, Open Work, Relevant Paths, Verification, Current State, Grounding Snapshot: Groundless Surfaces Validated Live, Provider Teardown Fix (+28 more)
...
### Community 156 - "MockWifiDriver"
Cohesion: 0.17
Nodes (10): HealthError, ResetError, RollbackError, MockWifiDriver, ActionId, Arc, AtomicBool, Default (+2 more)

### Community 186 - "resolve_local_model_path"
Cohesion: 0.24
Nodes (7): ConnectivityState, Path, PathBuf, ProviderConfig, expand_path(), resolve_local_model_path(), status_text()

## Knowledge Gaps
- **230 isolated node(s):** `Latest Snapshot`, `Sidebar Layout Design (2026-08-17)`, `Re-grounding Order`, `Snapshot Maintenance`, `The problem I'm trying to solve` (+225 more)
...
- **Why does `NodeId` connect `NodeId` to `tests.rs`, `Coordinator`, `security.rs`, `harness.rs`, `processes.rs`, `src/main.rs`, `discovery.rs`, `wifi.rs`, `power.rs`, `memory.rs`, `graphics.rs`, `storage.rs`, `drivers.rs`, `HealthState`, `network.rs`, `packages.rs`, `boot.rs`, `.chat_with_tools_outcome`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `ResourceId` connect `ResourceId` to `protocol.rs`, `security.rs`, `harness.rs`, `processes.rs`, `broker.rs`, `StagedExecutor`, `power.rs`, `memory.rs`, `graphics.rs`, `storage.rs`, `drivers.rs`, `network.rs`, `packages.rs`, `PrincipalId`, `boot.rs`, `MockWifiDriver`, `wifi_driver.rs`, `wifi.rs`, `MockPlanner`, `ToolRequest`, `.chat_with_tools_outcome`, `Coordinator`, `Checkpoint`, `.boot_with_probe`, `src/main.rs`, `ActionRecord`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `SystemGraph` connect `NodeId` to `Coordinator`, `security.rs`, `harness.rs`, `.boot_with_probe`, `src/main.rs`, `discovery.rs`, `processes.rs`, `EdgeId`, `power.rs`, `wifi.rs`, `memory.rs`, `graphics.rs`, `storage.rs`, `drivers.rs`, `HealthState`, `network.rs`, `packages.rs`, `boot.rs`?**
