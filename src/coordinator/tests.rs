...
fn surface_relay_returns_generated_html() {
    let handler = move |request: &str| {
        ...
        testutil::openai_response(
            "<section data-tauri-drag-region><span data-aios=\"used_percent\">42%</span></section>",
        )
    };
    let port = testutil::spawn_json_server(handler);
    let coordinator = stub_coordinator(port);
...
#[test]
fn chat_tool_loop_supports_multi_step_chaining() {
    // Stage 4: the planner may chain steps — a second tool call after the
    // first round's results must be executed, not forced into an answer.
    // Round 1: fetch. Round 2 (after seeing fetch result): health.
    // Round 3 (after seeing health result): grounded answer.
    let port = testutil::spawn_json_server(|body| {
        // Order matters: the accumulated transcript contains every earlier
        // "tool ... result" marker, so match the most recent round first.
        let health_rounds = body.matches("tool health result").count();
        if health_rounds >= 1 {
            testutil::openai_response("done: fetched docs and machine is healthy")
        } else if body.contains("tool web.fetch_url result") {
            testutil::openai_response(r#"{"tool_calls":[{"tool":"health","args":""}]}"#)
        } else {
            testutil::openai_response(
                r#"{"tool_calls":[{"tool":"web.fetch_url","args":"https://docs.rs/tokio"}]}"#,
            )
        }
    });
    let coordinator = stub_coordinator(port);
    let outcome = coordinator.chat_with_tools_outcome(vec![
        ModelMessage::new(ModelRole::System, "You are Aios."),
        ModelMessage::new(ModelRole::User, "pull tokio docs then check health"),
    ]);
    match &outcome {
        Ok(outcome) => eprintln!(
            "DEBUG answer={:?} tools={:?}",
    ...
    let answer = outcome.expect("chat").answer;
    assert_eq!(answer, "done: fetched docs and machine is healthy");
    let tool_entries: Vec<_> = coordinator
        .audit
        .filter("planner")
        .iter()
        .filter(|entry| entry.action == "tool")
        .cloned()
        .collect();
    assert!(
        tool_entries.len() >= 2,
        "expected at least two chained tool calls, got {}",
        tool_entries.len()
    );
}
...
fn wire_storage(coordinator: &mut Coordinator) {
    ...
    let principal = PrincipalId::agent(crate::storage::PACKAGE_ID, specialist.specialist.0.clone());
...
fn wire_network(coordinator: &mut Coordinator) {
    ...
    let principal = PrincipalId::agent(crate::network::PACKAGE_ID, specialist.specialist.0.clone());
    coordinator.broker.register_principal(
        principal.clone(),
        vec![
            Capability {
...
fn wire_drivers(coordinator: &mut Coordinator) {
    ...
    let principal = PrincipalId::agent(crate::drivers::PACKAGE_ID, specialist.specialist.0.clone());
    coordinator.broker.register_principal(
        principal.clone(),
        vec![
            Capability {
...
fn wire_memory(coordinator: &mut Coordinator) {
    ...
    let principal = PrincipalId::agent(crate::memory::PACKAGE_ID, specialist.specialist.0.clone());
    coordinator.broker.register_principal(
        principal.clone(),
        vec![
            Capability {
...
fn wire_power(coordinator: &mut Coordinator) {
    ...
    let principal = PrincipalId::agent(crate::power::PACKAGE_ID, specialist.specialist.0.clone());
    coordinator.broker.register_principal(
        principal.clone(),
        vec![
            Capability {
...
fn wire_boot_recovery(coordinator: &mut Coordinator) {
    ...
    let principal = PrincipalId::agent(crate::boot::PACKAGE_ID, specialist.specialist.0.clone());
    coordinator.broker.register_principal(
        principal.clone(),
        vec![
            Capability {
